        ...
        if ($domas->CODE_JOURNEE != 0) { 
            $final=$domas->CODE_JOURNEE;
              ...
              $bdd->beginTransaction();
                $vente= $bdd->prepare("SELECT V.DATE_VENTE, V.HEURE_VENTE,U.NOM_USER, U.PRENOM_USER, V.CODE_VENTE, V.STATUT, V.CODE_ENCAISSEMENT, E.CODE_USER, U1.NOM_USER NOM_ENCAISSEUR, U1.PRENOM_USER PRENOM_ENCAISSEUR FROM vente V JOIN utilisateur U ON V.CODE_USER = U.CODE_USER JOIN encaissement E ON V.CODE_ENCAISSEMENT = E.CODE_ENCAISSEMENT JOIN utilisateur U1 ON E.CODE_USER = U1.CODE_user WHERE V.CODE_JOURNEE =$final");
                $vente->execute();
            $bdd->commit(); 
?>
           ...
           <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Ventes de la journee </h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
